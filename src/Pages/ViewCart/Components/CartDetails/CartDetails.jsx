...
import {
  ...
  increment,
  setInitialQuantity,
} from "../../../../dataStoreComponent/viewcartSlice";
...
export default function CartDetails() {
  ...
  useEffect(() => {
    dispatch(setInitialQuantity(NumberOfQunt));
  ...
  const getAddons = async () => {
    try {
      setIsloading(true);
      let res = await axios.get(
        `https://api.vijayhomeservicebengaluru.in/api/userapp/getServiceAddOns`
      );
      if (res.status === 200) {
  ...
  const getDeliveryAddres = async () => {
    try {
      const response = await axios.get(
        "https://api.vijayhomeservicebengaluru.in/api/getcustomeraddress"
      );
      if (response.status === 200) {
